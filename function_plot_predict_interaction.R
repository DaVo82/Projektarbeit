                 ...
                 paste0(variable1, paste0(" and ", variable2))), time = NULL) 
{
  ...
  if (forest$treetype == "Regression") {
    ...
    plot <- ggplot(newdata, aes_string(x = variable1, y = variable2, 
                                       fill = "prediction")) + geom_raster() + theme_bw() + 
    ...
    if (ncol(pred) == 2) {
      newdata[, paste0("probability_", colnames(pred)[1])] <- pred[,1] #changed from -1 to 1 to get positives
    }
    else {
    ...
    if (is.null(time)) {
      time <- pred$unique.death.times[which.min(abs(colMeans(pred$survival, 
                                                             na.rm = TRUE) - 0.5))]
      message(sprintf("Using unique death time %s which is the closest to predicted median survival time.", 
                      time))
    }
