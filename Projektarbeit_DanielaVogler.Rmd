...
```{r data correlation, fig.cap="Korrelationskoeffizienten nach Pearson", fig.height=8, fig.width=12, message=TRUE, warning=FALSE, include = TRUE}
data_sample %>% 
...
mit Defaultparametern jedoch so tief, dass er nachträglich geprunt werden muss, 
um visuelle Erkenntnisse aus der Baumstruktur zu ermöglichen. Der geeignete 
Complexity-Parameter wurde durch manhuelles Ausprobierne bestimmt.
...
```{r resamples statistics, include = TRUE, fig.pos="H"}
...
knitr::kable(summary(results)$statistics$F, booktabs = T,
             caption = "F-Metrik") %>%
  kable_styling(latex_options = c("striped", "scale_down", "hold_position"),
                position = "center")

knitr::kable(summary(results)$statistics$AUC, booktabs = T,
             caption = "AUC") %>%
  kable_styling(latex_options = c("striped", "scale_down", "hold_position"),
                position = "center")

knitr::kable(summary(results)$statistics$Precision, booktabs = T,
             caption = "Precision") %>%
  kable_styling(latex_options = c("striped", "scale_down", "hold_position"),
                position = "center")

knitr::kable(summary(results)$statistics$Recall, booktabs = T,
             caption = "Recall") %>%
  kable_styling(latex_options = c("striped", "scale_down", "hold_position"),
                position = "center")
```
...
```{r variable importance plots, include = TRUE, fig.cap="Variablenwichtigkeit"}
p1 <- plot(varImp(cart_ds1), top = 20, main = "CART Baseline")

p2 <- plot(varImp(cart_ds2), top = 20, main = "CART Onset")

p3 <- plot(varImp(cart_ds3), top = 20, main = "CART Regression")

p4 <- plot(varImp(rf_ds1), top = 20, main = "Random Forest Baseline")

p5 <- plot(varImp(rf_ds2), top = 20, main = "Random Forest Onset")

p6 <- plot(varImp(rf_ds3), top = 20, main = "Random Forest Regression")

p7 <- plot(varImp(rf_nm_ds1), top = 20, 
           main = "Random Forest (non-missing) Baseline")

p8 <- plot(varImp(rf_nm_ds2), top = 20, 
           main = "Random Forest (non-missing) Onset")

p9 <- plot(varImp(rf_nm_ds3), top = 20, 
           main = "Random Forest (non-missing) Regression")

p10 <- plot(varImp(gbm_ds1), top = 20, main = "GBM Baseline")

p11 <- plot(varImp(gbm_ds2), top = 20, main = "GBM Onset")

p12 <- plot(varImp(gbm_ds3), top = 20, main = "GBM Regression")

grid.arrange(p1, p2, p3, ncol = 3, nrow = 4)
```
