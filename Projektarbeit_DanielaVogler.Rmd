...
Intensivpatienten in die Kategorien "Sepsis positiv" und "Sepsis negativ"
...
handhabt, müssen für einige andere Varianten von Entscheidungsbäumen und
Ensemblemethoden entweder fehlende Daten imputiert oder der Datensatz auf
vollständige Beobachtungen eingeschränkt werden. Dies gilt zum Beispiel für
Random Forests. Gradient Boosting Machines wiederum behandeln fehlende Werte
intern, indem sie diese als eigene Klasse beziehungsweise Wert definieren.
\newline
...
die Entwicklung jeder zeitabhängigen Prädiktorvariablen modelliert und der
...
\newline 
Dies entspricht einer regelmäßigen Klassifizierung der Patienten anhand 
...
Mittel aus Precision und Recall) an (inspiriert von @Brownlee).
...
späteren klinischen Anwendung ist somit gegeben.
...
`r knitr::kable(cart_ds1$bestTune, "latex", row.names = FALSE)`
...
`r knitr::kable(rf_ds1$bestTune, "latex", row.names = FALSE)`
...
`r knitr::kable(rf_nm_ds1$bestTune, "latex", row.names = FALSE)`
...
klassifizierte Beobachtungen zu verbessern. Hier wird nun ein Stochastic Gradient
Boosting mit dem R-Paket *gbm* durchgeführt. Dafür wird das Tunegrid wie folgt
definiert:
...
`r knitr::kable(gbm_ds1$bestTune, "latex", row.names = FALSE)`
...
`r knitr::kable(cart_ds2$bestTune, "latex", row.names = FALSE)`
...
`r knitr::kable(rf_ds2$bestTune, "latex", row.names = FALSE)`
...
`r knitr::kable(rf_nm_ds2$bestTune, "latex", row.names = FALSE)`
...
```
 ...
 `r knitr::kable(gbm_ds2$bestTune, "latex", row.names = FALSE)`
...
```
 ...
 `r knitr::kable(cart_ds3$bestTune, "latex", row.names = FALSE)`
...
`r knitr::kable(rf_ds3$bestTune, "latex", row.names = FALSE)`
...
`r knitr::kable(rf_nm_ds3$bestTune, "latex", row.names = FALSE)`
...
`r knitr::kable(gbm_ds3$bestTune, "latex", row.names = FALSE)`
...
wären Imputationsverfahren möglich, die kein Label voraussetzen.
...
# Unterordner zum Abspeichern anlegen, falls noch nicht vorhanden
if (!exists("figures")) {
  dir.create("figures")
}
...
Modelle dar. Dabei erhält die jeweils wichtigste Variable einen Score von 100 und 
...
*$\nobreak Bilirubin\_total$* folgt (der Variable, die im Random Forest des
...
herangezogen wird, sondern eine mit ihr korrelierende. Das könnte bedeuten, dass
...
Vorhersagewahrscheinlichkeit einer Sepsis erhöhen oder genau andersherum. Um dies
zu untersuchen, soll ein Vorhersage-Interaktions-Plot der beiden Variablen auf 
dem Modell betrachtet werden. Eine Funktion, um einen solchen Plot zu erstellen,
ist im R-Paket *randomForestExplainer* enthalten. Allerdings funktioniert diese
nicht für *ranger*-Objekte, die aus einem *caret::train*-Objekt extrahiert wurden.
Daher wurde der Code hier leicht modifiziert. Die Änderungen befinden sich in den
...
zuvor bereits der Kohlendioxidwert gesunken war, so kann dies als ernster Hinweis
auf eine fortschreitende Sepsis angesehen werden.\footnote{ \label{note1}
Medizinische Einordnung basierend auf einer Diskussion mit Dr. med. Ulf Dennler,
Leiter der Stabsstelle Strategisches Medizincontrolling am Universitätsklinikum
Ulm, Intensivmediziner, dem ich hiermit für seinen Beitrag danke.}
...
bereits ausgeführten Überlegungen zu diesen Variablen nichts ersichtlich. Die
...
```{r vars description, include = TRUE, fig.pos = "H"}
knitr::kable(var_info, format = "latex", booktabs = T,
...
```{r session info, include = TRUE, cache = FALSE, fig.pos = "H"}
pkgs <- data.frame(sessioninfo::package_info())[c("package", "loadedversion")]

n <- nrow(pkgs)
n2 <- ceiling(n/3)
n3 <- 2*n2

knitr::kable(list(pkgs[1:n2,], pkgs[(n2 + 1):n3,], pkgs[(n3 + 1):n,]),
             format = "latex",
             row.names = FALSE,
             col.names = c("Paket", "Version")) %>% 
