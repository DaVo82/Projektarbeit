...
header-includes:
  ...
  - \usepackage{xcolor}
  - \usepackage{hyphenat}
  - \hyphenation{Ent-wicklung Event-rate Intensiv-aufent-halts}
...
diverser Blutwerte und Vitalparameter, die im Rahmen von Intensivbehandlungen 
...
Baseline-Angaben zum Fall acht Vitalparameter (Variablen 3-10) und 26 Blutwerte
(Variablen 11-36), siehe Tabelle \ref{vars_table} im Anhang. 
\newline
...
Als Ausbruch gilt das Auftreten eines Organversagens gemäß Sequential Organ Failure
Assessment (SOFA) oder ein klinischer Verdacht, der sich in einer intravenösen
Antibiotikagabe und dem Anlegen von Blutkulturen widerspiegelt. Zu den Details 
siehe [@Physionet].
...
verzichtet, um den gezielt verkleinerten Datensatz nicht wieder erheblich zu 
...
*HospAdmTime* angepasst. Dieser wird mit Defaultparametern allerdings so tief, 
dass er nachträglich geprunt werden muss, um visuelle Erkenntnisse aus der
Baumstruktur zu ermöglichen. Der geeignete Complexity-Parameter wurde durch
manuelles Ausprobieren bestimmt und auf cp = 0,02 gesetzt. Es ergibt sich:
...
Splitvariable aus der Betrachtung entfernt wurde, werden nicht mit einbezogen, 
...
vor allem die Gradient Boosting Machine in Verbindung mit einem Regressionsansatz.
...
Prävalenz wenig aussagekräftig, da Werte in diesem Bereich bereits erreicht würden,
wenn immer negativ klassifiziert würde.
...
Vorhersage-Interaktions-Plot der beiden Variablen auf dem Modell betrachtet werden.
...
12, 13 und 44 (wobei die Änderung in Zeile 44 nicht zwingend notwendig ist):
...
ausgeführten Überlegungen zu diesen Variablen zunächst nicht viel ersichtlich. 
...
können. Dies deckt sich in etwa mit den obligaten Diagnosekriterien nach [@Werdan].

[evtl. Zusammenspiel Glukose-, Kalium- und Chloridänderung sowie Leukos 
...
reale Zeitscheibe statt zufällige Wahl bei Nicht-Sepsis]
