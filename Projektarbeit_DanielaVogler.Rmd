...
pred_rf_ds1 <- predict(rf_ds1$finalModel, rfImpute(Sepsis ~., test1)[,-1], type = "response")
confusionMatrix(data = pred_rf_ds1_test, reference = test1$Sepsis, positive = "positiv")
```
...
# Testdaten werden nicht automatisch imputiert
pred_rf_ds2 <- predict(rf_ds2$finalModel, rfImpute(Sepsis ~., test2)[,-1], type = "response")
confusionMatrix(data = pred_rf_ds2_test, reference = test2$Sepsis, positive = "positiv")
```
...
rf_ds3_final <- rf_ds3$finalModel

# Testdaten werden nicht automatisch imputiert
pred_rf_ds3 <- predict(rf_ds3$finalModel, rfImpute(Sepsis ~., test3)[,-1], type = "response")
confusionMatrix(data = pred_rf_ds3_test, reference = test3$Sepsis, positive = "positiv")
```
...
train1_2 <- train1[, which(colMeans(!is.na(train1)) > 0.7)] %>% 
  filter(complete.cases(.)) # na.action = na.omit in train funktioniert nur mit
                            # Formula-Methode, aber aufgrund des unterschiedlichen Umgangs
                            # mit Faktorvariablen funktioniert predict...finalModel nur
                            # mit x,y-Methode -> hier auf vollständige Fälle einschränken

rf_nm_ds1 <- train(x = train1_2[4:12],
                   y = train1_2$Sepsis,
                  method = "rf",
                  na.action = na.omit,
                  tuneGrid = mtry_grid,
...
pred_rf_nm_ds1 <- predict(rf_nm_ds1$finalModel, test1_2, type = "class")
...
rf_nm_ds2 <- train(x = train2_2[4:12],
                   y = train2_2$Sepsis,
                  method = "rf",
                  na.action = na.omit,
                  tuneGrid = mtry_grid,
...
pred_rf_nm_ds2 <- predict(rf_nm_ds2$finalModel, test2_2, type = "class")
...
rf_nm_ds3 <- train(x = train3_2[4:12],
                   y = train3_2$Sepsis,
                  method = "rf",
                  na.action = na.omit,
                  tuneGrid = mtry_grid,
...
pred_rf_nm_ds3 <- predict(rf_nm_ds3$finalModel, test3_2, type = "class")
...
results <- resamples(list(ds1_cart_raw = cart_ds1, ds1_cart_median = cart_proc_ds1,
                          ...
                          ds1_rf_rfImpute = rf_ds1, ds1_rf_nonmissing = rf_nm_ds1,
                          ds2_rf_rfImpute = rf_ds2, ds2_rf_nonmissing = rf_nm_ds2,
                           ds3_rf_rfImpute = rf_ds3, ds3_rf_nonmissing = rf_nm_ds3))
...
dotplot(results)
```
Achtung: Sensitivität und Spezifität sind vertauscht!
