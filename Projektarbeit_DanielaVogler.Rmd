...
[@BMSGPK]. Ich entscheide daher auch hier nach Vollständigkeit der
...
rpart.plot(cart_ds1$finalModel, type = 5, box.palette = "RdBu")
```
...
rpart.plot(cart_adj_ds1, type = 5, box.palette = "RdBu")
```
...
rpart.plot(cart_ds2$finalModel, type = 5, box.palette = "RdBu")
```
...
rpart.plot(cart_adj_ds2$finalModel, type = 5, box.palette = "RdBu")
```
...
rpart.plot(cart_ds3$finalModel, type = 5, box.palette = "RdBu")
```
...
rpart.plot(cart_adj_ds3$finalModel, type = 5, box.palette = "RdBu")
```
...
```{r resamples F statistics, include = TRUE, cache = FALSE}
...
```{r resamples AUC statistics, include = TRUE, cache = FALSE}
...
```{r resamples precision statistics, include = TRUE, cache = FALSE}
...
```{r resamples recall statistics, include = TRUE, cache = FALSE}
...
Forest und Gradient Boosting Machine) ähnliche Ergebnisse hinsichtlich der
F1-Metrik erreicht. Lediglich hinsichtlich der Präzision ist der CART-Algorithmus
deutlich schlechter.
\newline
Als erfolgversprechendste Modelle für eine weitere Erprobung ergeben sich die
...
Sensitivitäten zwischen 30 und 55 % und sehr gute Spezifitäten von 98 bis 99,9 %
erreicht.
...
kann, müssten die infrage kommenden Modelle nun auf verschiedenen Datensätzen aus
...
\newline
Interessant wäre hierbei auch, die Daten mehrfach zu zuvor festgelegten 
Zeitpunkten zu exportieren und bis zu diesem Stand zu analysieren, sodass die 
Notwendigkeit entfällt, wie in dieser Arbeit das Ende des Beobachtungszeitraums
für die Nicht-Sepsispatienten zufällig zu wählen.
...
Verschlechterung der Spezifität oder Präzision kommt, erstrebenswert. Zu diesem
...
\newline
Außerdem ist die Wahl der Evaluationsmetrik eine wichtige Stellschraube, die  
noch weitere Möglichkeiten bietet, zum Beispiel die Verwendung einer
...
Klassifikation geeignet sind und möglicherweise einen wesentlichen Beitrag zu dem
untersuchten Thema leisten können. Dies reicht von anderen Boosting-Algorithmen 
bis hin zu verschiedenen nicht baumbasierten Verfahren wie beispielsweise
neuronalen Netzen.
