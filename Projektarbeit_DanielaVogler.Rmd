...
dieser zeitnah an einer Sepsis erkranken wird beziehungsweise bereits unerkannt
erkrankt ist?
\newline
Hierfür werden exemplarisch CARTs (Entscheidungsbäume), Random Forests (Bagging)
und GBMs (Boosting) untersucht.
...
um den gezielt verkleinerten Datensatz nicht wieder erheblich zu vergrößern.
Dennoch ist festzuhalten, dass multiple Imputation eine mögliche Alternative
darstellt, die hinsichtlich einer eventuellen Verzerrung Vorteile aufweist.
...
testen. Hier wird auf die eingangs abgespalteten Testdatensätze mit 25 % der
...
Label bekannt und es ist zu befürchten, dass die tatsächliche Fehlerrate 
unterschätzt wird, wenn für den Test Daten imputiert werden, die Beobachtungen 
mit dem gleichen Sepsisstatus ähneln. An dieser Stelle wird daher multiple 
Imputation mit den Standardeinstellungen des R-Pakets *mice* verwendet.
...
# Für Random Forest zunächst Testdaten imputieren (ToDO: Sepsis ausschließen)
set.seed(12345)

test1_imputed <- complete(mice(test1, remove.collinear = FALSE), # TroponinI nicht
                                                                 # ausschließen
                          action = "stacked") 
...
# Testdaten imputieren (ToDo: Sepsis ausschließen)
set.seed(12345)

test2_imputed <- complete(mice(test2), action = "stacked") %>% 
  mutate(Unit2 = case_when(Unit1 == "ja"~ "nein",
                           Unit1 == "nein"~ "ja")
  )
...
test3_imputed <- complete(mice(test3), action = "stacked") %>% 
  mutate(Unit2 = case_when(Unit1 == "ja"~ "nein",
                           Unit1 == "nein"~ "ja")
         )
...
```{r confusion matrix plots1, include = TRUE, out.height='150%', out.width='120%', fig.cap="Konfusionsmatrizen (Baseline-Datensatz)", cache = FALSE}
grid.arrange(cf_table1, cf_table2, cf_table3, cf_table4,
             ncol = 2, nrow = 2)
```

```{r confusion matrix plots2, include = TRUE, out.height='150%', out.width='120%', fig.cap="Konfusionsmatrizen (Onset-Datensatz)", cache = FALSE}
grid.arrange( cf_table5, cf_table6, cf_table7, cf_table8, 
             ncol = 2, nrow = 2)
```

```{r confusion matrix plots3, include = TRUE, out.height='150%', out.width='120%', fig.cap="Konfusionsmatrizen (Regressionsdatensatz)", cache = FALSE}
...
Zu beachten ist auch, dass an dritter Stelle der Slope des 
$\nobreak (*Bilirubin_total*)$ folgt (der Variable, die im Random Forest des
Onset-Datensatzes als wichtigste bestimmt wurde), denn wie aus Abbildung
\ref{cor_plot} ersichtlich ist, korreliert *Bilirubin_total* vollständig mit dem
aus unseren Daten entfernten *Bilirubin_direct* und letzteres korreliert deutlich
negativ mit *EtCO2*. Der Korrelationskoeffizient für *EtCO2* und *Bilirubin_total*
selbst ist wenig aussagekräftig, weil es kaum Beobachtungen gibt, die für beide
Variablen Werte aufweisen. Wie im Kapitel zur Datenexploration bereits erwähnt, 
ist es möglich, dass für einen Split nicht die eigentlich relevante Variable
herangezogen wird, sondern eine mit ihr korrelierte. Das könnte bedeuten, dass 
nur einer der beiden hier diskutierten Prädiktoren wirklich bedeutsam für den
Random Forest ist, dies aber in umso höherem Maße. Falls dies zutrifft, ist zu
erwarten, dass entweder ein negativer *EtCO2*-Slope (Verringerung des Werts) und
ein positiver *Bilirubin_total*-Slope (Erhöhung des Werts) die
Vorhersagewahrscheinlichkeit einer Sepsis erhöhen oder genau andersherum. 
Um dies zu untersuchen, soll ein Vorhersage-Interaktions-Plot der beiden Variablen
auf dem Modell betrachtet werden. Eine Funktion, um einen solchen Plot zu
erstellen, ist im R-Paket *randomForestExplainer* enthalten. Allerdings
funktioniert diese nicht für *ranger*-Objekte, die aus einem *caret::train*-Objekt
extrahiert wurden. Daher wurde der Code hier leicht modifiziert. Die Änderungen
befinden sich in den Codezeilen 12, 13 und 44 (wobei die Änderung in Zeile 44 
