...
diverser Blutwerte und Vitalparameter, die üblicherweise im Rahmen von
Intensivbehandlungen erhoben werden, mittels prädiktiver Modelle bereits zu
erkennen, bevor sie klinisch diagnostiziert wird. Da der Einsatz von Machine
Learning-Algorithmen zur medizinischen Entscheidungsfindung nicht überall 
möglich und gewünscht ist, wird neben der Entwicklung von Vorhersagemodellen
auch die Frage untersucht, ob Entscheidungsregeln zur Unterstützung ärztlicher
Entscheidungen ableitbar sind.

Die bearbeitete Fragestellung lässt sich wie folgt zusammenfassen: \newline
1. Welche Entscheidungskriterien hinsichtlich einer Klassifikation von
Intensivpatienten in die Kategorien "wird eine Sepsis entwickeln" und "wird keine
Sepsis entwickeln" lassen sich mittels einzelner Entscheidungsbäume ableiten? \newline
2. Welches Modell aus dem Bereich der baumbasierten Ensemblemethoden ist am 
besten geeignet, um aus demografischen Daten, Blutwerten und Vitalparametern 
eines Intensivpatienten vorherzusagen, ob dieser an einer Sepsis erkranken wird
(bzw. bereits unerkannt erkrankt ist)?

Die verwendeten Daten stammen aus dem unter *Open Database License (ODbL)* 
verfügbaren Kaggle-Datensatz *Dataset.csv* [@KaggleData; @Physionet]. 
...
mit `r nrow(data_sample)` Beobachtungen zu 
`r length(unique(data_sample$Patient_ID))` Intensivpatienten verwendet. Die Daten
umfassen neben einigen demografischen und administrativen Baseline-Angaben zum
Fall acht Vitalparameter (Variablen 3-10) und 26 Blutwerte (Variablen 11-36), 
siehe Tabelle \ref{vars_table} im Anhang. 
...
innerhalb von sechs Stunden nach Aufnahme auf die Intensivstation erfolgt, wobei 
Ausbrüche innerhalb weniger als vier Stunden aus den Daten entfernt wurden. Als 
Ausbruch gilt das Auftreten eines Organversagens gemäß Sequential Organ Failure
Assessment (SOFA) oder ein klinischer Verdacht, der sich in einer intravenösen
Antibiotikagabe und dem Anlagen von Blutkulturen widerspiegelt.
...
daher zwischen 0 und 1 liegen. Werte außerhalb dieses Bereichs wurden daher 
entfernt.
...
Aufnahmezeitpunkt_Intensiv - Aufnahmezeitpunkt_gesamt) vorgenomen worden. Hier
wurde auf positive Vorzeichen vereinheitlicht, da diese für die Interpretation 
...
Nun analysieren wir die Daten nun auf Korrelationen zwischen den numerischen
Variablen und betrachten dazu eine Heatmap der Korrelationskoeffizienten nach
Pearson:
...
müssen stark korrelierte Prädiktoren nicht unbedingt von der Analyse
...
sondern eine, die stark mit ihr korreliert ist.

Bei Variablen mit vollständigen linearen Zusammenhang 
(Korrelationskoeffizient +/- 1) ist eine Variablenselektion aber auf jeden Fall
angeraten:
...
Da Temperatur und Blutwerte häufig nur einmal täglich bestimmt wurden, werden 
...
dass bestimmte Messungen (insbesondere die Bestimmung spezieller Blutwerte) nur 
...
in Random Forests konstruiert [@Breiman]. Auf multiple Imputation wird hier 
verzichtet, um den gezeilt verkleinerten Datensatz nicht wieder erheblich zu 
vergrößern. Dennoch ist festzuhalten, dass multiple Imputation eine mögliche
Alternative darstellt, die hinsichtlich einer eventuellen Verzerrung Vorteile
aufweist.
...
die Entwicklung jeder Prädiktorvariable modelliert und der patientenindividuelle
Slope als Prädiktor verwendet. Dabei wird für Nicht-Sepsispatienten der gesamte
Beobachtungszeitraum einbezogen und für Sepsispatienten der Zeitraum bis zum 
ersten positiven Outcome.
...
unausgewogene Daten sind aber Metriken, die auf Precision und Recall beruhen, von
Vorteil. Daher bieten sich hier die *Area under the Precision Recall Curve (AUC)* 
und die F1-Metrik (harmonisches Mittel aus Precision und Recall) an. 
...
werden. Eine flexible Anpassung beispielsweise im Rahmen einer späteren klinischen
Anwendung ist also gegeben.
...
ist kein Preprocessing notwendig. Das Tuning erfolgt mittels der
...
Als bestes Modell ergibt sich: `r knitr::kable(cart_ds1$bestTune)`
...
(*HospAdmTime*) eine Rolle spielt, erscheint einleuchtend. Da hier jedoch gar keine
weiteren Variablen herangezogen werden, wird noch ein Baum ohne *HospAdmTime*
angepasst. Dieser wird mit Defaultparametern allerdings so tief, dass er
nachträglich geprunt werden muss, um visuelle Erkenntnisse aus der Baumstruktur 
zu ermöglichen. Der geeignete Complexity-Parameter wurde durch manuelles
Ausprobieren bestimmt und auf CP = 0,02 gesetzt.
...
Als bestes Modell ergibt sich: `r knitr::kable(rf_ds1$bestTune)`
...
als 30 % fehlenden Werten eingeschränkt und anschließend  nur die vollständigen
Beobachtungen einbezogen. Dabei ist zu beachten, dass das Tunegrid für den
Parameter *mtry* (d. h. die Anzahl der pro Split einbezogenen möglichen Splitvariablen) auf maximal (Anzahl der Variablen - 1) beschränkt wird.
...
`r knitr::kable(gbm_ds1$bestTune)`
...
die Tunegrids bleiben mit Ausnahme der nötigen Anpassung von *mtry* bei Verringerung
der Variablen gleich. 
...
`r knitr::kable(cart_ds2$bestTune)`
...
vorigen Abschnitt wird nun auch hier diese Variable ausgeschlossen:
...
rpart.plot(cart_adj_ds2$finalModel, type = 5)
```
...
Und schließlich für die Gradient Boosting Machine:
...
```
 ...
 `r knitr::kable(gbm_ds2$bestTune)`
...
```
 ...
 `r knitr::kable(cart_ds3$bestTune)`
...
Wiederum im finalen Modell nur die Dauer des Intensivaufenthalts als
...
Random Forests mit Imputation führen zu:
...
`r knitr::kable(rf_nm_ds3$bestTune)`
...
Und Gradient Boosting führt hier zu:
...
`r knitr::kable(gbm_ds3$bestTune)`
...
der drei Datensätze (also Baseline, Onset und Regression). Die weiteren 
angepassten Entscheidungsbäume, bei denen jeweils die zunächst verwendete 
Splitvariable aus der Betrachtung entfernt wurden, werden nicht mit einbezogen, 
da sie weniger der verbesserung der Vorhersagequalität als vielmehr der Beurteilung
relevanter Kriterien dienen sollen.
...
zeitliche Abstand zum Event besteht. Auch der Erfolg des Regressionsdatensatzes
leuchtet ein, da hier die meisten Informationen im Sinne des zeitlichen Verlaufs 
der Werte eingehen. Unerwartet kommt hingegen das bessere Abschneiden des einzelnen
...
`r prettyNum(round(summary(results)$statistics$F[3,4] -
                   summary(results)$statistics$F[9,4], 
                   4),
            decimal.mark = ",")`). 
...
kein Konfidenzintervall der F1-Metrik angegeben werden kann, da die Metrik auf vier 
der fünf Folds nicht berechnet werden konnte, weil Precision und Recall jeweils Null
sind.
...
aus, dass die Präzision im Vergleich zu den anderen Modellen eher schlecht, der 
Recall aber sehr gut ist (mit 
`r prettyNum(round(summary(results)$statistics$Recall[3,4], 3), decimal.mark = ",")`
der zweitbeste Wert).

Für eine klinische Anwendung empfiehlt sich demzufolge von den untersuchten Modellen
vor allem die Gradient Boosting Machine in Verbindung mit einem Regressionsmodell.
...
Alternative dar, wenn der Präzision im Vergleich zum Recall eher wenig Bedeutung
...
GBM mit F1 = 
`r prettyNum(round(summary(results)$statistics$F[10,4], 3), decimal.mark = ",")`,
Precision = 
`r prettyNum(round(summary(results)$statistics$Precision[10,4], 3), 
            decimal.mark = ",")` 
und Recall = 
`r prettyNum(round(summary(results)$statistics$Recall[10,4], 3), 
             decimal.mark = ",")`).
...
testen. Wir greifen hier auf unsere eingangs abgespalteten Testdatensätze mit 25 %
der Beobachtungen des ursprünglichen Samples zurück - in dem Bewusstsein, dass 
diese aus den gleichen Datenquellen stammen wie unsere Trainingsdaten. Wir erhalten
...
# Testdaten werden nicht automatisch imputiert (außerhalb Preprocessing)
pred_rf_ds1 <- predict(rf_ds1, rfImpute(Sepsis ~., test1)[-1])
...
Alle Modelle mit Ausnahme des CART und des GBM auf dem Baseline-Datensatz weisen 
hohe Accuracys von über 90 % auf; allerdings ist dies aufgrund der niedrigen
Prävalenz wenig aussagekräftig, da Werte in diesem Bereich bereits erreicht werden,
wenn immer negativ klassifiziert wird.
...
Vorgehen zu einer Spezifität von ebenfalls nur ca. 50 % führen. Wir können also
...
hoch genug ist, lässt sich nicht eindeutig beantworten. Ein entsprechender Grenzwert
sollte daher in jeder Anwendungssituation individuell mit den beteiligten
...
Sensitivität (= Recall) des Random Forests auf dem imputierten Onset-Datensatz auf,
aufgrund derer dieses Modell nun aus den Empfehlungen herausfällt.
...
Imputation auf dem Regressionsdatensatz trotz seiner niedrigeren Sensitivität je
nach Anwendungssituation vorzuziehen sein.

## Entscheidungsregeln und Einfluss einzelner Prädiktoren

Die Plots am Ende diesesAbschnitts stellen die Variablenwichtigkeit der einzelnen
...
Verbesserung des Gini-Unreinheitsmaßes.
...
die Intensivstation kommen, jedoch noch nicht notwendigerweise diagnostiziert sind, 
während die zuerst genannte Gruppe die Sepsis erst auf der ITS erwirbt. Dennoch 
muss der Intensivaufenthalt nicht notwendigerweise kausal für die Erkrankung sein. 
Möglich ist zum Beispiel auch, dass Personen mit einem längeren Intensivaufenthalt 
schwerer krank und somit in einem schlechteren Allgemeinzustand sind, der das 
Entstehen einer Sepsis begünstigt. Ähnlich könnte auch Abbildung \ref{cart_ds1}
interpretiert werden: Wer sehr schnell (innerhalb der ersten eineinhalb Stunden)
oder erst spät (nach mehr als 45 Stunden) auf die ITS verlegt wurde, ist in einem
schlechteren Zustand als die übrigen Intensivpatienten und hat möglicherweise 
deshalb ein höheres Sepsisrisiko.

Die große Bedeutung der beiden Prädiktoren kann auch stark davon beinflusst sein,
...
der Variablenwichtigkeit und der Entscheidungsbäume ansieht. Insgesamt scheinen 
die Vitalparameter (Blutdruckwerte, Herz- und Atemfrequenz, Temperatur und
...
Schauen wir uns die beiden favorisiereten Modelle genauer an:

Die Gradient Bossting Machine mit dem Regressionsansatz wird stärker als jedes
...
auf der ITS aber die mit Abstand größte Bedeutung zukommt.

Bei dem auf  dem Tetsdatensatz wegen seiner sehr hohen Spezifität positiv
...
Zubeachten ist auch, dass an dritter Stelle der Slope des *Bilirubin_total*
folgt (der Variable, die im Random Forest das Onset-Datensatzes als wichtigste
...
sondern eine mit ihr korrelierte. Das könnte bedeuten, dass nur einer der beiden
...
*Bilirubin_total*-Slope (Erhöhung des Werts) die Vorhersagewahrscheinlichkeit einer
Sepsis erhöhen oder genau andersherum. Um dies zu untersuchen, soll ein
Vorhersage-Interaktions-Plot bei beiden Variablen auf dem Modell betrachtet werden.
Eine Funktion, um einen solchen Plot zu erstellen, ist im R-Paket
*randomForestExplainer* enthalten. Allerdings funktioniert diese nicht für
*ranger*-Objekte, die aus einem *caret::train*-Objekt extrahiert wurden. Daher habe
ich den Code leicht modifiziert. Die Änderungen befinden sich in den Codezeilen 
12, 13 und 43 (wobei die Änderung in Zeile 43 nicht zwingend notwendig):

```{r plot_predict_interaction function, attr.source = ".numberLines", include = TRUE, echo = TRUE, cache = FALSE}
...
plot_predict_interaction.caret_ranger(rf_ds3$finalModel,ds3_imputed, 
                                      "slope_Bilirubin_total", "slope_EtCO2",
                                      main = "")
```

Tatsächlich ist zu erkennen, dass die Vorhersagewahrscheinlichkeit für eine Sepsis
am höchsten ist, wenn der *Bilirubin_total*-Wert steigt, während der *EtCO2*-Wert
abnimmt.

Die CART-Bäume wurden hauptsächlich mit dem Ziel angepasst, Entscheidungsregeln
beziehungsweise -pfade abzuleiten. Da aus dem Tuning entstandenen Bäume nur die
Dauer des Intensivaufenthalts beziehungsweise die Zeit von der Hospitalisierung
bis zur Verlegung auf die Intensivstation abbilden, ist aus ihnen außer den bereits 
ausgeführten Überlegungen zu diesen Variablen zunächst nicht viel ersichtlich. Die
Auswertungen der Variablenwichtigkeit und die weiteren generierten
...
können. Dies deckt sich in etwa mit den obligaten Diagnosekriterien nach
[@Sepsisbuch].

[evtl. Zusammenspiel Glukose-, Kalium- und Chloridänderung Medizinische Einordnung?]

```{r variable importance plot1, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit CART Baseline"}
plot(varImp(cart_ds1), top = 20, xlab = "")
```

```{r variable importance plot2, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit CART Onset"}
plot(varImp(cart_ds2), top = 20, xlab = "")
```

```{r variable importance plot3, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit CART Regression"}
plot(varImp(cart_ds3), top = 20, xlab = "")
```

```{r variable importance plot4, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit  Random Forest (imputiert) Baseline"}
plot(varImp(rf_ds1), top = 20, xlab = "")
```

```{r variable importance plot5, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit Random Forest (imputiert) Onset"}
plot(varImp(rf_ds2), top = 20, xlab = "")
```

```{r variable importance plot6, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit  Random Forest (imputiert) Regression"}
plot(varImp(rf_ds3), top = 20, xlab = "")
```

```{r variable importance plot7, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit Random Forest (non-missing) Baseline"}
plot(varImp(rf_nm_ds1), top = min(20, n_var1), xlab = "")
```

```{r variable importance plot8, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit  Random Forest (non-missing) Onset"}
plot(varImp(rf_nm_ds2), top = min(20, n_var2), xlab = "")
```

```{r variable importance plot9, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit  Random Forest (non-missing) Regression"}
plot(varImp(rf_nm_ds3), top = min(20, n_var3), xlab = "")
```

```{r variable importance plot10, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit GBM Baseline"}
plot(varImp(gbm_ds1), top = 20, xlab = "")
```

```{r variable importance plot11, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit  GBM Onset"}
plot(varImp(gbm_ds2), top = 20, xlab = "")
```

```{r variable importance plot12, include = TRUE, fig.cap="TOP 20 Variablenwichtigkeit  GBM Regression"}
...
[andere Modelle
andere Imputationsmethoden und/oder vollständigere Daten
