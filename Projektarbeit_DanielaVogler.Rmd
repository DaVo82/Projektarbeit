...
genauer Abgrenzung der Diagnosen bei um die 50% und höher. Eine frühzeitige Erkennung 
und damit einhergehend eine rechtzeitige Therapie gelten als wichtigste Mittel zur
...
outcome <- outcome %>% 
  mutate(Sepsis = factor(Sepsis, levels = c(1, 0),
                         labels = c("positiv", "negativ") # "positiv" muss 1. Klasse sein, damit caret
                         )                                # später die Gütemaße geeignet zuordnet
         )
...
müssen wir uns hier keine allzu großen Sorgen um die Korrelationen machen. Allerdings 
sollten starke Kollinearitäten später bei der Interpretation der Entscheidungskriterien 
berücksichtigt werden. Es ist möglich, dass nicht die eigentlich relevante Variable 
für einen Split herangezogen wird, sondern eine, die stark mit ihr korreliert ist.

Bei Variablen mit vollständigen linearen Zusammenhang (Korrelationskoeffizient +/- 1) 
ist eine Variablenselektion dennoch angeraten:

Hour dient nur zur Information und soll nicht als Einflussvariable verwendet werden.
Daher kann sie hier vernachlässigt werden.
...
direkten Bilirubins, wird die Variable Bilirubin_direct aus dem Datensatz entfernt.
...
Hämoglobin- und Hämatokrit-Wert entsprechen einander ungefähr, weil die Erythrozyten 
den Großteil des Gesamtvolumens der Blutzellen ausmachen  \footnote{https://www.gesundheit.gv.at/labor/laborwerte/blutbild/haematokrit-hkt}.
Ich entscheide daher auch hier nach Vollständigkeit der Beobachtungen und entferne 
die Variable Hgb aus dem Datensatz.
...
Zum einen fehlen in vielen Variablen sehr viele Werte. Es muss also ein Weg gefunden 
werden, dennoch gute Ergebnisse zu erhalten.

Zum anderen müssen die longitudinalen Daten für die Anwendung Baum-basierter Verfahren 
zu je einer Beobachtung pro Patient zusammengefasst werden. Dazu werden drei Ansätze
...
die Entwicklung jeder einbezogenen Variable modelliert und der patientenindividuelle 
Slope als Prädiktor verwendet. Dabei wird für Nicht-Sepsispatienten der gesamte 
Beobachtungszeitraum einbezogen und für Sepsispatienten der Zeitraum bis zum ersten 
positiven Outcome.
...
Die drei entstandenen Datensätze werden anschließend jeweils in einen Trainings- und Validierungsdatensatz mit 75% der Beobachtungen und einen Testdatensatz mit
...
Aufgrund der sehr unausgewogenen Daten (Eventrate %) wird ein Upsampling vorgenommen.
...
```{r caret rf model ds1, include = TRUE}
# set.seed(12345)
# 
...
```
```{r caret rf model ds1 ranger, include = TRUE}
set.seed(12345)
...
confusionMatrix(data = pred_rfr_ds1, reference = test1$Sepsis, positive = "positiv")
```

```{r caret rf model ds2, include = TRUE,}
# set.seed(12345)
# 
# # Imputation nach Breiman:
# 
...
```
```{r caret rf model ds2 ranger, include = TRUE}
set.seed(12345)
...
confusionMatrix(data = pred_rfr_ds2, reference = test2$Sepsis, positive = "positiv")
```

```{r caret rf model ds3, include = TRUE}
# set.seed(12345)
# 
# # Imputation nach Breiman:
# 
...
```
```{r caret rf model ds3 ranger, include = TRUE}
set.seed(12345)
...
confusionMatrix(data = pred_rfr_ds3, reference = test3$Sepsis, positive = "positiv")
```
Als Alternative soll  versucht werden, nur die Variablen mit relativ wenig fehlenden Werten (< 30%) als Prädiktoren zu verwenden und dann nur vollständige Beobachtungen einzubeziehen.
...
confusionMatrix(data = pred_rf_nm_ds1, reference = test1_2$Sepsis, positive = "positiv")
```
...
confusionMatrix(data = pred_rf_nm_ds2, reference = test2_2$Sepsis, positive = "positiv")
```
...
confusionMatrix(data = pred_rf_nm_ds3, reference = test3_2$Sepsis, positive = "positiv")
```
...
gbm_ds1 <- train(x = train1[,4:41],
                  y = train1$Sepsis,
                  method = "gbm",
                  distribution = "bernoulli", # Adaboost und Huber führen zu mehr richtig positiven,
                                              # aber auch sehr vielen falsch positiven
                  metric = "ROC",
                  tuneGrid = gbm_grid,
                  trControl = ctrl,
                  verbose = FALSE)
...
confusionMatrix(data = pred_gbm_ds1, reference = test1$Sepsis, positive = "positiv")
```
...
confusionMatrix(data = pred_gbm_ds2, reference = test2$Sepsis, positive = "positiv")
```
...
gbm_ds3 <- train(x = train3[,3:40],
                  y = train3$Sepsis,
                  method = "gbm",
                  distribution = "bernoulli", # Huber nicht besser, Adaboost schlechter
                  metric = "ROC",
                  tuneGrid = gbm_grid,
                  trControl = ctrl,
                  verbose = FALSE)
...
confusionMatrix(data = pred_gbm_ds3, reference = test3$Sepsis, positive = "positiv")
```
